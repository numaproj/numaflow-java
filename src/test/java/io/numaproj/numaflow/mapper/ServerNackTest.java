...
public class ServerNackTest {
    ...
    public void setUp() throws Exception {
        ...
        GRPCConfig grpcServerConfig = GRPCConfig.newBuilder()
                ...
                .build();
        server = new Server(grpcServerConfig, new NackMapFn(), null, serverName);
        server.start();
    ...
    private static class NackMapFn extends Mapper {
        @Override
        public MessageList processMessage(String[] keys, Datum datum) {
            return MessageList.newBuilder()
                    .addMessage(Message.toNack(NackOptions.newBuilder().delay(500L).reason("retry").build()))
                    .build();
        }
    }
}
